# python /home/francesco/data_scratch/swiss-ndvi-processing/demo/script/L1_continous_gapfilling.py
    ...
    if np.isfinite(new_data):
        ...
        if (
            ...
            (new_data < lower[i] - iqr[i] * param_iqr)
                ...
                (new_delta - last_delta < 0.0005) or 
                (new_delta - last_delta > 0.005)
                ):
        ...
        if outlier_new == 0:
            ...
            if potential_position > last_position:
            ...
            if i - last_position == 1:
                # no gapfilling because are continous
...
flat_array = np.concatenate([np.atleast_1d(x) for x in new_data_list])
...
ax.scatter(dates_sorted[101:500], flat_array, s=10, color="black", label="observed value", zorder=3)
...
plot_results(pixel, outlier_arr, ndvi_gapfilled, ndvi_sorted, lower, upper,
             save_path=f"demo/output/pixel_{pixel}_gapfilled.png")
