import numpy as np
import matplotlib.pyplot as plt
import torch
import torch.nn as nn
import math
from scipy.signal import savgol_filter
from scipy.ndimage import gaussian_filter1d
import statsmodels.api as sm
import pandas as pd
import datetime

def gapfill_ndvi(
    ndvi_series,
    lower,
    upper,
    forecasting=False,
    param_iqr=1.5,
    bottom_q=0.3,
    top_q=0.7,
    return_quantiles = False,
    weight_median = 0.5,
    y_delta_l = 0,
    y_delta_h = 0,
    y_iqr = 0,
    r_delta_l = 0,
    r_delta_h = 0,
    r_iqr = 0,
    smoothing_method = "savgol",
    window_smoothing = 14,
    sigma = 4,
    frac = 0.2,
    lag_forecast = 14,
    use_tau = False,
    tau = 9999
):
    """
    NDVI gapfilling.

    Steps:
      1. Normalize & mask clouds.
      2. Detect outliers vs logistic model bounds.
      3. Remove outliers.
      4. Fill gaps using interpolation guided by logistic model median.

    Modes:
      - forecasting=False: behaves like gapfill_timeseries
          returns ndvi_gapfilled, outlier_mask
      - forecasting=True : behaves like sequential_gapfill
          returns ndvi_filled, outlier_mask, forecast_only
    """
    # --- normalize ---
    ndvi_series = ndvi_series.astype(float) / 10000.0
    ndvi_series = np.where((ndvi_series > 1) | (ndvi_series < 0), np.nan, ndvi_series)
    n = len(ndvi_series)
    outlier_mask = np.zeros(n, dtype=bool)
    iqr_param = np.full(n, np.nan)
    forecast_only = np.full(n, np.nan)
    smoothed = np.full(n, np.nan)
    original_obs = ndvi_series.copy()


    def fill_segment(start, end):
        # Only fill if there is a gap larger than 1
        if end - start > 1:

            # for larg gaps this is safer
            if end - start > 30:

                medians =  upper[start + 1: end] - (upper[start + 1: end] - lower[start + 1: end]) / 2
                median_end = upper[end] - (upper[end] - lower[end]) / 2
                median_start = upper[start] - (upper[start] - lower[start]) / 2

                deltas = (ndvi_gapfilled[end]-median_end) - (ndvi_gapfilled[start]-median_start)
                ndvi_gapfilled[start + 1: end] = medians + deltas

            else:

                for j, idx in enumerate(range(start + 1, end)):
                    frac = (j + 1) / (end - start)

            
                    # linear interpolation between observations
                    obs_interp = ndvi_gapfilled[start] + (ndvi_gapfilled[end] - ndvi_gapfilled[start]) * frac
                    
                    # logistic model median
                    median_val = upper[idx] - (upper[idx] - lower[idx]) / 2
                    
                    # bias toward median, max at center not working with large gaps
                    diff = median_val - obs_interp
                    weight = frac * (1 - frac)
                    
                    ndvi_gapfilled[idx] = obs_interp + weight_median * weight * diff

    # --------------------
    #  outlier detection
    # --------------------
    valid_idx = np.where(np.isfinite(ndvi_series))[0]
    if len(valid_idx) < 2:
        if forecasting:
            return ndvi_series.copy(), outlier_mask, np.full(n, np.nan)
        else:
            return ndvi_series.copy(), outlier_mask
    # window_size = odd integer for local thresholds (use 5 or 7)
    window_size = 21

    valid_ndvi = ndvi_series[valid_idx]
    valid_upper = upper[valid_idx]
    valid_lower = lower[valid_idx]
    valid_iqr = valid_upper - valid_lower
    median_valid = (valid_upper + valid_lower) / 2.0  

    # -----------------------
    # Global threshold band
    # -----------------------
    iqr_param = np.where(valid_ndvi / median_valid < 1, median_valid / valid_ndvi, valid_ndvi / median_valid)

    th_hi = valid_upper + np.quantile(iqr_param,0.7) * valid_iqr
    th_lo = valid_lower - np.quantile(iqr_param,0.7) * valid_iqr
    is_outlier_global = (valid_ndvi > th_hi) | (valid_ndvi < th_lo)

    # -----------------------
    # Local rolling threshold (to catch local sharp dips)
    # -----------------------
    deltas = valid_ndvi - median_valid  
    th_hi_delta = th_hi - median_valid
    th_lo_delta = th_lo - median_valid

    # -----------------------
    # Local rolling threshold (on deltas)
    # -----------------------
    n = valid_ndvi.size
    half = window_size // 2
    local_th_hi_delta = np.empty(n)
    local_th_lo_delta = np.empty(n)

    for i in range(n):
        a = max(0, i - half)
        b = min(n, i + half + 1)
        window_deltas = deltas[a:b]
        if window_deltas.size >= 3:
            q1, q3 = np.quantile(window_deltas, [0.3, 0.6])
            iqr_local = q3 - q1
            local_th_hi_delta[i] = q3 + param_iqr * iqr_local
            local_th_lo_delta[i] = q1 - param_iqr * iqr_local
        else:
            # fallback to global delta thresholds if window too small
            local_th_hi_delta[i] = th_hi_delta[i]
            local_th_lo_delta[i] = th_lo_delta[i]

    # apply in delta-space
    is_outlier_local = (deltas > local_th_hi_delta) | (deltas < local_th_lo_delta)

    # -----------------------
    # Slope-based detection
    # -----------------------
    deltas = valid_ndvi - median_valid                # shape (n,)
    delta_diff = np.diff(deltas)                      # shape (n-1,)
    slope_outlier = np.zeros(n, dtype=bool)
    valid_outlier = np.zeros(len(deltas), dtype = bool)

    if delta_diff.size >= 2:
        q_low, q_hi = np.quantile(delta_diff, [0.2, 0.8])

        # same-direction extremes: both previous and next step extreme (same sign allowed)
        same_dir = (
            ((delta_diff[1:] > q_hi) | (delta_diff[1:] < q_low))
            & ((delta_diff[:-1] > q_hi) | (delta_diff[:-1] < q_low))
        )  # shape (n-2,)

        # V-shape: previous and next extreme with opposite signs (drop then rise, or rise then drop)
        vshape = (
            ((delta_diff[1:] > q_hi) & (delta_diff[:-1] < q_low))
            | ((delta_diff[1:] < q_low) & (delta_diff[:-1] > q_hi))
        )  # shape (n-2,)

        # single-step *very* extreme:
        q_low_ext, q_hi_ext = np.quantile(delta_diff, [0.1, 0.9])
        single_step = (delta_diff > q_hi_ext) | (delta_diff < q_low_ext)  # shape (n-1,)

        # For a point i (1..n-2) we consider:
        # - same_dir or vshape involving delta_diff[i-1] and delta_diff[i]
        # - OR if either neighboring step is a single very-extreme step
        slope_outlier[1:-1] = (
            same_dir | vshape | single_step[:-1] 
        )
        
        #  check the first/last positions using the single-step rule:
        slope_outlier[0] = single_step[0] 
        slope_outlier[-1] = single_step[-1]  

    # -----------------------
    # Final outlier mask (must be slope-outlier AND (global OR local threshold outlier))
    # -----------------------
    is_outlier = slope_outlier & (is_outlier_global | is_outlier_local)
    # -----------------------
    # Extra extreme-value research
    # -----------------------

    if delta_diff.size >= 2:
        q_low_dd, q_hi_dd = np.quantile(delta_diff, [0.1, 0.9])
        extreme_delta = (delta_diff < q_low_dd) | (delta_diff > q_hi_dd)

        # expand extreme_delta to point indices
        extra_outlier = np.zeros_like(slope_outlier)
        extra_outlier[1:] |= extreme_delta
        extra_outlier[:-1] |= extreme_delta
    else:
        extra_outlier = np.zeros_like(slope_outlier)

    ratio = np.maximum(valid_ndvi, median_valid) / np.minimum(valid_ndvi, median_valid)
    q_hi_ratio = np.quantile(ratio, 0.95)
    extreme_ratio = ratio > q_hi_ratio

    # Combine
    extreme_outlier = extra_outlier | extreme_ratio

    # Merge into final outlier mask
    is_outlier = is_outlier | extreme_outlier


    inside_band = (valid_ndvi >= valid_lower) & (valid_ndvi <= valid_upper)
    is_outlier = is_outlier & ~inside_band

    outlier_mask[valid_idx] = is_outlier
    ndvi_series[valid_idx[is_outlier]] = np.nan

    # --------------------------
    # Gapfilling on full time serie
    # --------------------------
    if not forecasting:
        
        ndvi_gapfilled = ndvi_series.copy()
        if len(valid_idx) < 2:
            return ndvi_gapfilled, outlier_mask

        for i in range(len(valid_idx) - 1):
            start, end = valid_idx[i], valid_idx[i + 1]
            fill_segment(start,end)

        # After gapfilling is complete (non-forecasting branch)
        smoothed = np.full_like(ndvi_gapfilled, np.nan)

        # optional smoothing
        if smoothing_method is not None:
            if smoothing_method == "savgol":
                wl = window_smoothing if (window_smoothing % 2 == 1) else (window_smoothing + 1)
                maxlen = len(ndvi_gapfilled)
                if maxlen % 2 == 0:
                    maxlen -= 1
                wl = min(wl, maxlen) if maxlen >= 3 else wl
                if wl >= 3:
                    smoothed[:] = savgol_filter(ndvi_gapfilled, window_length=wl, polyorder=2)

            elif smoothing_method == "low_pass":
                smoothed[:] = gaussian_filter1d(ndvi_gapfilled, sigma=sigma)

            elif smoothing_method == "loess":
                index = np.arange(len(ndvi_gapfilled))
                # drop NaNs before fitting
                valid = np.isfinite(ndvi_gapfilled)
                loess_smooth = sm.nonparametric.lowess(
                    ndvi_gapfilled[valid], index[valid], frac=frac, return_sorted=True
                )
                # interpolate smoothed values back onto full index
                smoothed[:] = np.interp(index, loess_smooth[:, 0], loess_smooth[:, 1])

        valid_outlier = is_outlier

        if not return_quantiles:
            return ndvi_gapfilled, outlier_mask, smoothed, valid_outlier, valid_idx, deltas
        else:
            return ndvi_gapfilled, outlier_mask, q_hi, q_low, delta_diff, iqr_param, smoothed, valid_outlier,valid_idx,deltas


    # forecasted mode

    else:  

        n = len(original_obs)

        # outputs (reset for comparison)
        outlier_mask = np.zeros(n, dtype=bool)
        ndvi_gapfilled = original_obs.copy()
        forecast_only = np.full(n, np.nan)
        smoothed = np.full(n, np.nan)

        last_idx = None             # last confirmed observation
        last_potential_idx = None   # last potential outlier

        for t in range(n):

            obs = original_obs[t]

            if np.isfinite(obs) and obs > 0:
                # thresholds


                inside_band = (obs >= lower[t]) and (obs <= upper[t])

                if not inside_band:

                    median_t = 0.5 * (upper[t] + lower[t])

                    ratio = np.maximum(obs, median_t) / np.minimum(obs, median_t)

                    #print(f"{t=}, {obs=}, {lower[t]=}, {upper[t]=}, {ratio=}")


                    # local rolling check (last 7 valid + non-outlier obs)
                    recent = ndvi_gapfilled[:t]              # all previous values up to t
                    recent_mask = (~outlier_mask[:t]) & np.isfinite(ndvi_gapfilled[:t])
                    medians = 0.5* (upper[:t][recent_mask] - lower[:t][recent_mask])
                    recent = ndvi_gapfilled[:t][recent_mask]

                    local_outlier = False

                    if recent.size >= 14:

                        recent = recent[-14:]
                        medians = medians[-14:]
                        recent = recent - medians
                        q1, q3 = np.quantile(recent, [0.3, 0.7])
                        
                        if obs - median_t > q3 or obs - median_t < q1:
                            
                            local_outlier = True
                            local_outlier = False

                    potential_now = ratio > y_iqr  #(ratio > y_iqr or local_outlier)

                    # slope check (delta-delta)
                    delta_delta = 0
                    if last_idx is not None and np.isfinite(ndvi_gapfilled[last_idx]):
                        median_last = 0.5 * (upper[last_idx] + lower[last_idx])
                        delta_prev = ndvi_gapfilled[last_idx] - median_last
                        delta_curr = obs - median_t
                        delta_delta = (delta_curr - delta_prev)

                    # -----------------------
                    # Decision rules
                    # -----------------------
                    
                    # Strict outlier (extreme values)
                    extreme_outlier = (
                            (delta_delta > r_delta_h) or (delta_delta < r_delta_l) or (ratio > r_iqr)
                        )

                        # Potential outlier
                    potential_outlier = (
                            (delta_delta > y_delta_h) or (delta_delta < y_delta_l)  or potential_now
                        )

                    if extreme_outlier:
                            
                            outlier_mask[t] = True
                            ndvi_gapfilled[t] = np.nan
                            forecast_only[t] = np.nan

                    # if delta delta and params is outlier
                    elif potential_outlier:

                       last_potential_idx = t

                    # != is xor operator
                    #elif potential_outlier_delta_delta != potential_now:

                    # no idea why extra check is needed
                    if inside_band:
                        outlier_mask[t] = False
                        ndvi_gapfilled[t] = obs
        

                else:
                    # confirmed observation
                    ndvi_gapfilled[t] = obs
                    outlier_mask[t] = False

                    # resolve pending potential
                    # --- retroactive resolution of potential outlier ---
                    if last_potential_idx is not None and last_potential_idx > (last_idx or -1):

                        p = last_potential_idx
                        raw_p = ndvi_series[p]
                        median_p = 0.5 * (upper[p] + lower[p])
                        median_t = 0.5 * (upper[t] + lower[t])
                        delta_p = raw_p - median_p
                        delta_t = ndvi_gapfilled[t] - median_t
                        delta_delta_p = delta_t - delta_p

                        # safety: check band first
                        inside_band_p = (raw_p >= lower[p]) and (raw_p <= upper[p])

                        if (not inside_band_p) and ((delta_delta_p > y_delta_h or delta_delta_p < y_delta_l)):
                            # confirmed outlier (only if outside band!)
                            outlier_mask[p] = True
                            ndvi_gapfilled[p] = np.nan
                            #if last_idx is not None:
                                #fill_segment(last_idx, t)
                        else:
                            
                            # accept retroactively
                            outlier_mask[p] = False
                            #fill_segment(last_idx, p)
                            #fill_segment(p, t)
                            last_idx = t

                        last_potential_idx = None

                    # gapfill between last confirmed obs and this obs
                    #if last_idx is not None:
                        #fill_segment(last_idx, t)

                    last_idx = t


        else:
        
                # missing obs -> forecast continuation
                if last_idx is not None and np.isfinite(ndvi_gapfilled[last_idx]):
                    median_t = 0.5 * (upper[t] + lower[t])
                    delta_last = ndvi_gapfilled[last_idx] - 0.5 * (upper[last_idx] + lower[last_idx])
                    if use_tau:
                        decrease_factor = math.exp(-math.log(2) * ((t-last_idx) / tau))
                    else:
                        decrease_factor = 1
                    forecast_val = median_t + delta_last * decrease_factor
                    forecast_only[t] = forecast_val

        return ndvi_gapfilled, outlier_mask, forecast_only, smoothed




def plot_results(title, ndvi_series, ndvi_gapfilled, outlier_arr, lower, upper, dates, plot_points = True, show_iqr = False,param_iqr = 1.5, save_path=None):
    """
    Plot NDVI time series for a pixel:
      - raw NDVI with outliers highlighted
      - logistic model bounds
      - gapfilled NDVI
    """
    colors = np.where(outlier_arr, "red", "green")

    fig, ax = plt.subplots(figsize=(12, 6))

    # logistic bounds
    ax.plot(dates, lower, label="Lower Bound", color="red", alpha=0.7)
    ax.plot(dates, upper, label="Upper Bound", color="red", alpha=0.7)
    ax.fill_between(dates, lower, upper, alpha=0.2, color="red")

    # raw NDVI with outliers marked
    ax.scatter(dates, ndvi_series, s=10, color=colors, label="Raw NDVI", zorder=3)

    # gapfilled NDVI
    if plot_points:
        ax.plot(dates, ndvi_gapfilled, color="black", label="Gapfilled NDVI")



    ax.set_title(title)
    ax.set_ylim(-0.1, 1.0)
    ax.set_xlabel("Date")
    ax.set_ylabel("NDVI")
    ax.legend()
    plt.tight_layout()

    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches="tight")

def double_logistic_function(t, params):
    sos, mat_minus_sos, sen, eos_minus_sen, M, m = torch.split(params, 1, dim=1)
    mat_minus_sos = torch.nn.functional.softplus(mat_minus_sos)
    eos_minus_sen = torch.nn.functional.softplus(eos_minus_sen)
    sigmoid_sos_mat = torch.nn.functional.sigmoid(
        -2 * (2 * sos + mat_minus_sos - 2 * t) / (mat_minus_sos + 1e-10)
    )
    sigmoid_sen_eos = torch.nn.functional.sigmoid(
        -2 * (2 * sen + eos_minus_sen - 2 * t) / (eos_minus_sen + 1e-10)
    )
    return (M - m) * (sigmoid_sos_mat - sigmoid_sen_eos) + m


def ndvi_continous(
    ndvi_series,
    lower,
    upper,
    y_delta_l = 0,
    y_delta_h = 0,
    y_iqr = 0,
    r_delta_l = 0,
    r_delta_h = 0,
    r_iqr = 0,
    frac = 0.05,
    tau = 14
    ):

    # --- normalize ---
    ndvi_series = ndvi_series.astype(float) / 10000.0
    ndvi_series = np.where((ndvi_series > 1) | (ndvi_series < 0), np.nan, ndvi_series)
    n = len(ndvi_series)
    outlier_mask = np.zeros(n, dtype=bool)
    forecast_only = np.full(n, np.nan)
    deltas = np.full(n, np.nan)
    smoothed = np.full(n, np.nan)
    original_obs = ndvi_series.copy()
    ndvi_gapfilled = ndvi_series.copy()

    last_idx = None             # last confirmed observation
    last_potential_idx = None   # last potential outlier

    for t in range(n):

        obs = original_obs[t]

        if np.isfinite(obs) and obs > 0:
            # thresholds

            inside_band = (obs >= lower[t]) and (obs <= upper[t])

            median_t = 0.5 * (upper[t] + lower[t])

            deltas[t] = obs - median_t

            if not inside_band:

                ratio = np.maximum(obs, median_t) / np.minimum(obs, median_t)

                potential_now = ratio > y_iqr  

                # slope check (delta-delta)
                delta_delta = 0
                if last_idx is not None and np.isfinite(ndvi_gapfilled[last_idx]):
                    median_last = 0.5 * (upper[last_idx] + lower[last_idx])
                    delta_prev = ndvi_gapfilled[last_idx] - median_last
                    delta_curr = obs - median_t
                    delta_delta = (delta_curr - delta_prev)

                # -----------------------
                # Decision rules
                # -----------------------
                
                # Strict outlier (extreme values)
                extreme_outlier = (
                        (delta_delta > r_delta_h) or (delta_delta < r_delta_l) or (ratio > r_iqr)
                    )

                # Potential outlier
                potential_outlier = (
                        ((delta_delta > y_delta_h) or (delta_delta < y_delta_l))  and potential_now
                    )

                if extreme_outlier:
                        
                    outlier_mask[t] = True
                    ndvi_gapfilled[t] = np.nan
                    forecast_only[t] = np.nan

                # if delta delta and params is outlier
                elif potential_outlier:

                    last_potential_idx = t

                # no idea why extra check is needed
                if inside_band:
                    outlier_mask[t] = False
                    ndvi_gapfilled[t] = obs
        
            else:
                # confirmed observation
                ndvi_gapfilled[t] = obs
                outlier_mask[t] = False
                
                # resolve pending potential
                # --- retroactive resolution of potential outlier ---
                if last_potential_idx is not None and last_potential_idx > (last_idx or -1):

                    p = last_potential_idx
                    raw_p = ndvi_series[p]
                    median_p = 0.5 * (upper[p] + lower[p])
                    median_t = 0.5 * (upper[t] + lower[t])
                    delta_p = raw_p - median_p
                    delta_t = ndvi_gapfilled[t] - median_t
                    delta_delta_p = delta_t - delta_p

                    # safety: check band first
                    inside_band_p = (raw_p >= lower[p]) and (raw_p <= upper[p])

                    if (not inside_band_p) and ((delta_delta_p > y_delta_h or delta_delta_p < y_delta_l)):
                        # confirmed outlier (only if outside band!)
                        outlier_mask[p] = True
                        ndvi_gapfilled[p] = np.nan
                        #if last_idx is not None:
                            #fill_segment(last_idx, t)
                    else:
                        
                        # accept retroactively
                        outlier_mask[p] = False
                        deltas[p] = delta_p
                        #fill_segment(last_idx, p)
                        #fill_segment(p, t)
                        last_idx = t

                    last_potential_idx = None

                last_idx = t

        else:
        
            # missing obs -> forecast continuation
            if last_idx is not None and np.isfinite(ndvi_gapfilled[last_idx]):

                median_t = 0.5 * (upper[t] + lower[t])
                delta_last = ndvi_gapfilled[last_idx] - 0.5 * (upper[last_idx] + lower[last_idx])
                decrease_factor = math.exp(-math.log(2) * ((t-last_idx) / tau))

                forecast_val = median_t + delta_last * decrease_factor
                forecast_only[t] = forecast_val

    return ndvi_gapfilled, outlier_mask, forecast_only, smoothed, deltas 


import datetime



    # Double logistic function
def double_logistic_function_numpy(date, params):
    # Convert date to DOY
    if isinstance(date, (datetime.date, datetime.datetime, np.datetime64, pd.Timestamp)):
        doy = pd.Timestamp(date).dayofyear
    else:
        raise ValueError("date must be a datetime or Timestamp")

    # Scale as in original torch code
    T_SCALE = 1.0 / 365.0
    t = torch.tensor([[doy]], dtype=torch.float32) * T_SCALE

    sos, mat_minus_sos, sen, eos_minus_sen, M, m = torch.split(params, 1, dim=1)
    mat_minus_sos = torch.nn.functional.softplus(mat_minus_sos)
    eos_minus_sen = torch.nn.functional.softplus(eos_minus_sen)

    sigmoid_sos_mat = torch.sigmoid(
        -2 * (2 * sos + mat_minus_sos - 2 * t) / (mat_minus_sos + 1e-10)
    )
    sigmoid_sen_eos = torch.sigmoid(
        -2 * (2 * sen + eos_minus_sen - 2 * t) / (eos_minus_sen + 1e-10)
    )

    return (M - m) * (sigmoid_sos_mat - sigmoid_sen_eos) + m



def ndvi_continous_final(
    df,
    date,
    last_date,
    last_potential_dates,
    deltas_arr,
    dates_delta_arr,
    params_upper,
    params_lower,
    y_delta_l=0.02,
    y_delta_h=0.02,
    y_iqr=0.1,
    r_delta_l=-0.1,
    r_delta_h=0.1,
    r_iqr=0.3,
    tau=14,
    smoothing_values = 7,
    use_real_idx = False,
    frac = 1
):

    if smoothing_values % 2 == 1:
        middle = int((smoothing_values + 1) / 2 -1)
    else:
        middle = int(smoothing_values / 2 -1)

    obs = df.at[date, "ndvi"]

    # thresholds
    upper = double_logistic_function_numpy(date, params_upper).item()
    lower = double_logistic_function_numpy(date, params_lower).item()
    median_t = 0.5 * (upper + lower)

    df.at[date, "upper"] = upper
    df.at[date, "lower"] = lower

    if np.isfinite(obs) and obs > 0:

        inside_band = (obs >= lower) and (obs <= upper)

        outlier = False

        if not inside_band:

            ratio = np.maximum(obs, median_t) / np.minimum(obs, median_t)
            potential_now = ratio > y_iqr

            delta_delta = 0

            if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)
                delta_prev = df.at[last_date, "ndvi"] - median_last
                delta_curr = obs - median_t
                delta_delta = delta_curr - delta_prev

            # rules
            extreme_outlier = (
                (delta_delta > r_delta_h) or (delta_delta < r_delta_l) or (ratio > r_iqr)
            )

            potential_outlier = (
                ((delta_delta > y_delta_h) or (delta_delta < y_delta_l)) and potential_now
            )

            if extreme_outlier:
                df.at[date, "outlier"] = True
                df.at[date, "deltas"] = np.nan
                outlier = True

            elif potential_outlier:
                last_potential_dates.append(date)
                outlier = True
            
        if not outlier:

            # confirmed obs
            delta_curr = obs - median_t
            df.at[date, "outlier"] = False
            df.at[date, "deltas"] = delta_curr

            deltas_arr.append(delta_curr)
            dates_delta_arr.append(date)

            if len(deltas_arr) > smoothing_values:
                deltas_arr = deltas_arr[-smoothing_values:]
                dates_delta_arr = dates_delta_arr[-smoothing_values:]

            # perform deltas L1
            if last_date is not None:

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)
                delta_prev = df.at[last_date, "ndvi"] - median_last
                
                days_diff = (date - last_date).days
                L1_deltas = np.linspace(delta_prev, delta_curr, num = days_diff +1)
                df.loc[last_date:date, "deltas_L1"] = L1_deltas

            # retroactive check
            potential_between = 0

            if last_potential_dates:

                last_known_date = date

                for pot_date in reversed(last_potential_dates):

                    last_upper =  double_logistic_function_numpy(pot_date, params_upper).item()
                    last_lower =  double_logistic_function_numpy(pot_date, params_lower).item()
                    median_potential_last = 0.5 * (last_upper + last_lower)

                    # read data
                    delta_potential_prev = df.at[pot_date, "ndvi"] - median_potential_last
                    delta_potential = delta_potential_prev - median_potential_last

                    # calculate from the most recent obs (may vary backwards)
                    last_upper_obs =  double_logistic_function_numpy(last_known_date, params_upper).item()
                    last_lower_obs =  double_logistic_function_numpy(last_known_date, params_lower).item()
                    median_last_known = 0.5 * (last_upper_obs + last_lower_obs)

                    # read data
                    delta_t = df.at[last_known_date, "ndvi"] - median_last_known
                    delta_delta_p = delta_t - delta_potential

                    if  (delta_delta_p > y_delta_h) or (delta_delta_p < y_delta_l):

                        # confirm outlier
                        df.at[pot_date, "outlier"] = True
                        df.at[pot_date, "deltas"] = np.nan

                    else:

                        # accept as true value
                        df.at[pot_date, "outlier"] = False

                        potential_between = potential_between + 1
                        # insert between current date and last known obs
                        deltas_arr.insert(len(dates_delta_arr) -1 - 2* potential_between, delta_potential)
                        dates_delta_arr.insert(len(dates_delta_arr) -1 - 2* potential_between, pot_date)

                        last_known_date = pot_date
                        
                # perform smoothing from last_known date to last_pot date

                values_to_keep = max(smoothing_values, 3 +2 * potential_between)
                
                # clean the last_potential date
                last_potential_dates.clear()
   
            if potential_between > 0 and len(deltas_arr) >= smoothing_values:


                deltas_arr = deltas_arr[-values_to_keep:]
                dates_delta_arr = dates_delta_arr[- values_to_keep:]

                # Convert list of dates to DatetimeIndex
                dates_delta_arr_dt = pd.to_datetime(dates_delta_arr)

                if use_real_idx:
                    idx = (dates_delta_arr_dt - dates_delta_arr_dt[0]).days
                else:
                    idx = np.arange(len(dates_delta_arr_dt))

                # LOESS smoothing over the full window
                loess = sm.nonparametric.lowess(deltas_arr, idx, frac= smoothing_values /len(dates_delta_arr), return_sorted=True)

                start_date = dates_delta_arr_dt[2 * potential_between -1]
                end_date = dates_delta_arr_dt[-2]
                dates_interp = pd.date_range(start=start_date, end=end_date, freq='D')
                dates_interp = dates_interp.intersection(df.index)

                if use_real_idx:
                    x_interp = (dates_interp - dates_delta_arr_dt[0]).days
                else:
                    x_interp = np.linspace(idx[2 * potential_between -1], idx[-2], len(dates_interp))

                # Interpolate LOESS values
                y_interp = np.interp(x_interp, loess[:, 0], loess[:, 1])
                # Assign smoothed values
                mask = df.loc[dates_interp, "delta_smoothed"].isna()
                if mask.any():
                    df.loc[dates_interp[mask], "delta_smoothed"] = y_interp[mask.values]
            
            else:

                # Write data: perform smoothing from last_date to current date
                if len(dates_delta_arr) == smoothing_values:

                    # Convert list of dates to DatetimeIndex
                    dates_delta_arr_dt = pd.to_datetime(dates_delta_arr)

                    if use_real_idx:
                        idx = (dates_delta_arr_dt - dates_delta_arr_dt[0]).days
                    else:
                        idx = np.arange(len(dates_delta_arr_dt))

                    # LOESS smoothing over the full window
                    loess = sm.nonparametric.lowess(deltas_arr, idx, frac=frac, return_sorted=True)

                    # Dates to interpolate: from second-to-last to last, inclusive
                    start_date = dates_delta_arr_dt[middle -1]
                    end_date = dates_delta_arr_dt[middle]
                    dates_interp = pd.date_range(start=start_date, end=end_date, freq='D')
                    dates_interp = dates_interp.intersection(df.index)

                    if use_real_idx:
                        x_interp = (dates_interp - dates_delta_arr_dt[0]).days
                    else:
                        x_interp = np.linspace(idx[middle -1], idx[middle], len(dates_interp))

                    # Interpolate LOESS values
                    y_interp = np.interp(x_interp, loess[:, 0], loess[:, 1])
                    # Assign smoothed values
                    df.loc[dates_interp, "delta_smoothed"] = y_interp

            # update the last known date
            last_date = date

        else:
            # esitmate NDVI based on last obs
            if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)

                # read data
                delta_prev = df.at[last_date, "ndvi"] - median_last
                days_diff = (date - last_date).days

                decrease_factor = math.exp(-math.log(2) * (days_diff / tau))
                forecast_val = median_t + delta_prev * decrease_factor

                # write data
                df.at[date, "forecast"] = forecast_val

    else:
        # esitmate NDVI based on last obs
        if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

            last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
            last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
            median_last = 0.5 * (last_upper + last_lower)

            # read data
            delta_prev = df.at[last_date, "ndvi"] - median_last
            days_diff = (date - last_date).days

            decrease_factor = math.exp(-math.log(2) * (days_diff / tau))
            forecast_val = median_t + delta_prev * decrease_factor

            # write data
            df.at[date, "forecast"] = forecast_val

    return last_date, last_potential_dates, deltas_arr, dates_delta_arr



def ndvi_continous_final_2(
    df,
    date,
    last_date,
    last_potential_date,
    deltas_arr,
    dates_delta_arr,
    params_upper,
    params_lower,
    y_delta_l=0.02,
    y_delta_h=0.02,
    y_iqr=0.1,
    r_delta_l=-0.1,
    r_delta_h=0.1,
    r_iqr=0.3,
    tau=14,
    smoothing_values = 7,
    use_real_idx = False,
    frac = 1,
    latency = [],
    current_date_latency = [],
):

    if smoothing_values % 2 == 1:
        middle = int((smoothing_values + 1) / 2 -1)
    else:
        middle = int(smoothing_values / 2 -1)

    obs = df.at[date, "ndvi"]

    # thresholds
    upper = double_logistic_function_numpy(date, params_upper).item()
    lower = double_logistic_function_numpy(date, params_lower).item()
    median_t = 0.5 * (upper + lower)

    df.at[date, "upper"] = upper
    df.at[date, "lower"] = lower

    if np.isfinite(obs) and obs > 0:

        inside_band = (obs >= lower) and (obs <= upper)

        outlier = False
        potential_outlier = False

        if not inside_band:
            ratio = np.maximum(obs, median_t) / np.minimum(obs, median_t)
            potential_now = ratio > y_iqr

            delta_delta = 0

            if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)
                delta_prev = df.at[last_date, "ndvi"] - median_last
                delta_curr = obs - median_t
                delta_delta = delta_curr - delta_prev

            # rules
            extreme_outlier = (
                (delta_delta > r_delta_h) or (delta_delta < r_delta_l) or (ratio > r_iqr)
            )
            potential_outlier = (
                ((delta_delta > y_delta_h) or (delta_delta < y_delta_l)) and potential_now
            )

            if extreme_outlier:
                df.at[date, "outlier"] = True
                df.at[date, "deltas"] = np.nan
                outlier = True
            
        if not outlier:

            if not potential_outlier:

                # confirmed obs
                delta_curr = obs - median_t
                df.at[date, "outlier"] = False
                df.at[date, "deltas"] = delta_curr

                deltas_arr.append(delta_curr)
                dates_delta_arr.append(date)

                if len(deltas_arr) > smoothing_values:
                    deltas_arr = deltas_arr[-7:]
                    dates_delta_arr = dates_delta_arr[-7:]

            # check for potential outlier
            if last_potential_date is not None:

                last_upper =  double_logistic_function_numpy(last_potential_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_potential_date, params_lower).item()
                median_potential_last = 0.5 * (last_upper + last_lower)

                # read data
                delta_potential_prev = df.at[last_potential_date, "ndvi"] - median_potential_last
                delta_potential = delta_potential_prev - median_potential_last

                # read data
                delta_delta_p = median_t - delta_potential

                potential_outlier_2 = (delta_delta_p > y_delta_h) or (delta_delta_p < y_delta_l)

                if  potential_outlier_2:

                    # confirm outlier
                    df.at[last_potential_date, "outlier"] = True
                    df.at[last_potential_date, "deltas"] = np.nan

                else:

                    # accept as true value
                    df.at[last_potential_date, "outlier"] = False
                    df.at[date, "outlier"] = False
                    potential_outlier = False

                    # insert between current date and last known obs
                    deltas_arr.insert(len(dates_delta_arr) -1, delta_potential)
                    dates_delta_arr.insert(len(dates_delta_arr) -1, last_potential_date)

                                    
                    # perform smoothing from last_known date to last_pot date
                    if len(deltas_arr) > smoothing_values:
                        deltas_arr = deltas_arr[-7:]
                        dates_delta_arr = dates_delta_arr[-7:]

                    if len(deltas_arr) == smoothing_values:

                        # Convert list of dates to DatetimeIndex
                        dates_delta_arr_dt = pd.to_datetime(dates_delta_arr)

                        if use_real_idx:
                            idx = (dates_delta_arr_dt - dates_delta_arr_dt[0]).days
                        else:
                            idx = np.arange(len(dates_delta_arr_dt))

                        # LOESS smoothing over the full window
                        loess = sm.nonparametric.lowess(deltas_arr, idx, frac = frac, return_sorted=True)

                        start_date = dates_delta_arr_dt[middle -1 ]
                        end_date = dates_delta_arr_dt[middle]
                        dates_interp = pd.date_range(start=start_date, end=end_date, freq='D')
                        dates_interp = dates_interp.intersection(df.index)

                        if use_real_idx:
                            x_interp = (dates_interp - dates_delta_arr_dt[0]).days
                        else:
                            x_interp = np.linspace(idx[middle -1], idx[middle], len(dates_interp))

                        # Interpolate LOESS values
                        y_interp = np.interp(x_interp, loess[:, 0], loess[:, 1])

                        # Assign smoothed values
                        df.loc[dates_interp, "delta_smoothed"] = y_interp

                    # perform deltas L1
                    if last_date is not None:

                        last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                        last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                        median_last = 0.5 * (last_upper + last_lower)
                        delta_prev = df.at[last_date, "ndvi"] - median_last
                        
                        days_diff = (date - last_date).days
                        L1_deltas = np.linspace(delta_prev, delta_curr, num = days_diff +1)
                        df.loc[last_date:date, "deltas_L1"] = L1_deltas
                    
                
                # clean the last_potential date
                if potential_outlier:
                    last_potential_date = date
                else:
                    last_potential_date = None

            else:

                # Write data: perform smoothing from last_date to current date
                if len(dates_delta_arr) == smoothing_values:

                    # Convert list of dates to DatetimeIndex
                    dates_delta_arr_dt = pd.to_datetime(dates_delta_arr)

                    if use_real_idx:
                        idx = (dates_delta_arr_dt - dates_delta_arr_dt[0]).days
                    else:
                        idx = np.arange(len(dates_delta_arr_dt))

                    # LOESS smoothing over the full window
                    loess = sm.nonparametric.lowess(deltas_arr, idx, frac=frac, return_sorted=True)

                    # Dates to interpolate: from second-to-last to last, inclusive
                    start_date = dates_delta_arr_dt[middle -1 ]
                    end_date = dates_delta_arr_dt[middle]
                    dates_interp = pd.date_range(start=start_date, end=end_date, freq='D')
                    dates_interp = dates_interp.intersection(df.index)

                    if use_real_idx:
                        x_interp = (dates_interp - dates_delta_arr_dt[0]).days
                    else:
                        x_interp = np.linspace(idx[middle -1], idx[middle], len(dates_interp))

                    # Interpolate LOESS values
                    y_interp = np.interp(x_interp, loess[:, 0], loess[:, 1])
                    # Assign smoothed values
                    df.loc[dates_interp, "delta_smoothed"] = y_interp

                    days_diff = (date - dates_delta_arr[3]).days
                    latency.append(days_diff)
                    current_date_latency.append(date)

            # perform deltas L1
            if last_date is not None:

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)
                delta_prev = df.at[last_date, "ndvi"] - median_last
                        
                days_diff = (date - last_date).days
                L1_deltas = np.linspace(delta_prev, delta_curr, num = days_diff +1)
                df.loc[last_date:date, "deltas_L1"] = L1_deltas
            # update the last known date
            last_date = date

        else:
            # esitmate NDVI based on last obs
            if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)

                # read data
                delta_prev = df.at[last_date, "ndvi"] - median_last
                days_diff = (date - last_date).days

                decrease_factor = math.exp(-math.log(2) * (days_diff / tau))
                forecast_val = median_t + delta_prev * decrease_factor

                # write data
                df.at[date, "forecast"] = forecast_val

    else:

        df.at[date, "outlier"] = True
        # esitmate NDVI based on last obs
        if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

            last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
            last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
            median_last = 0.5 * (last_upper + last_lower)

            # read data
            delta_prev = df.at[last_date, "ndvi"] - median_last
            days_diff = (date - last_date).days

            decrease_factor = math.exp(-math.log(2) * (days_diff / tau))
            forecast_val = median_t + delta_prev * decrease_factor

            # write data
            df.at[date, "forecast"] = forecast_val

    return last_date, last_potential_date, deltas_arr, dates_delta_arr, latency, current_date_latency

# almost working
"""def ndvi_continous_final(
    df,
    date,
    last_date,
    last_potential_date,
    params_upper,
    params_lower,
    y_delta_l=0.02,
    y_delta_h=0.02,
    y_iqr=0.1,
    r_delta_l=-0.1,
    r_delta_h=0.1,
    r_iqr=0.3,
    tau=14,
):

    obs = df.at[date, "ndvi"]

    # thresholds
    upper = double_logistic_function_numpy(date, params_upper).item()
    lower = double_logistic_function_numpy(date, params_lower).item()
    median_t = 0.5 * (upper + lower)

    df.at[date, "upper"] = upper
    df.at[date, "lower"] = lower

    if np.isfinite(obs) and obs > 0:
        inside_band = (obs >= lower) and (obs <= upper)
        df.at[date, "deltas"] = obs - median_t

        outlier = False

        if not inside_band:
            ratio = np.maximum(obs, median_t) / np.minimum(obs, median_t)
            potential_now = ratio > y_iqr

            delta_delta = 0

            if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

                last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
                median_last = 0.5 * (last_upper + last_lower)
                delta_prev = df.at[last_date, "ndvi"] - median_last
                delta_curr = obs - median_t
                delta_delta = delta_curr - delta_prev

            # rules
            extreme_outlier = (
                (delta_delta > r_delta_h) or (delta_delta < r_delta_l) or (ratio > r_iqr)
            )
            potential_outlier = (
                ((delta_delta > y_delta_h) or (delta_delta < y_delta_l)) and potential_now
            )

            if extreme_outlier:
                df.at[date, "outlier"] = True
                df.at[date, "deltas"] = np.nan
                outlier = True

            elif potential_outlier:
                last_potential_date = date
                outlier = True
            

        if not outlier:

            # confirmed obs
            df.at[date, "outlier"] = False

            # retroactive check
            if last_potential_date is not None:

                last_upper =  double_logistic_function_numpy(last_potential_date, params_upper).item()
                last_lower =  double_logistic_function_numpy(last_potential_date, params_lower).item()
                median_potential_last = 0.5 * (last_upper + last_lower)

                delta_potential_prev = df.at[last_potential_date, "ndvi"] - median_potential_last

                delta_potential = delta_potential_prev - median_potential_last
                delta_t = obs - median_t
                delta_delta_p = delta_t - delta_potential

                if  (delta_delta_p > y_delta_h) or (delta_delta_p < y_delta_l):

                    # confirm outlier
                    df.at[last_potential_date, "outlier"] = True
                    df.at[last_potential_date, "deltas"] = np.nan

                    # perform smoothing from last date to date

                else:

                    # accept as true value
                    df.at[last_potential_date, "outlier"] = False

                    # perform smoothing from last date to last potential date and from last potential date to date

                # clean the last_potential date
                last_potential_date = None

            # update the last known date
            last_date = date

    else:
        # esitmate NDVI based on last obs
        if last_date is not None and np.isfinite(df.at[last_date, "ndvi"]):

            last_upper =  double_logistic_function_numpy(last_date, params_upper).item()
            last_lower =  double_logistic_function_numpy(last_date, params_lower).item()
            median_last = 0.5 * (last_upper + last_lower)

            # read last value
            delta_prev = df.at[last_date, "ndvi"] - median_last
            days_diff = (date - last_date).days

            decrease_factor = math.exp(-math.log(2) * (days_diff / tau))
            forecast_val = median_t + delta_prev * decrease_factor
            # write NDVI estimation
            df.at[date, "forecast"] = forecast_val

    return last_date, last_potential_date"""

