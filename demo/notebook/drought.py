#   nohup python -u /home/francesco/data_scratch/swiss-ndvi-processing/demo/notebook/drought.py >  /home/francesco/data_scratch/swiss-ndvi-processing/demo/output/log/drought.log 
# 2>/dev/null &
...
full_index = full_index[(full_index <= "2019-10-01")]
...
for i, pixel_sel in enumerate(sel):
    ...
    df = df.loc[df.index <= pd.Timestamp("2019-10-01")]
    ...
    for date in df.index[df.index <= pd.Timestamp("2019-10-01")]:
...
out_gif_combined_1 = "/home/francesco/data_scratch/swiss-ndvi-processing/demo/output/gif/drought.mp4"

# Prepare writers (stream to disk instead of keeping frames in memory)
writer1 = imageio.get_writer(out_gif_combined_1, fps=10, format='ffmpeg', codec='libx264', ffmpeg_params=['-pix_fmt','yuv420p','-crf','18','-preset','fast'], quality = 8)
