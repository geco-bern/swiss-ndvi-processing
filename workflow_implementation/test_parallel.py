# nohup python -u /home/francesco/data_scratch/swiss-ndvi-processing/workflow_implementation/test_parallel.py > /home/francesco/data_scratch/swiss-ndvi-processing/demo/output/log/parallel.log 
...
OUTPUT_DIR = "/data_2/scratch/francesco/zarr_demo_daily_processed/"
N_FILES = 20
N_PIXELS_PER_FILE = 100
DAYS_TO_PROCESS = 1500   # matches your earlier loops; change to None to process all
...
# L1_interpolation (keeps identical semantics to your original)
...
# L2_smoothing - full function as in your original script
def L2_smoothing(pixel_idx, init_position, params_lower, params_upper):
    ...
    base_date = zarr_date_to_date(dates_list[0])
    for d in last_dates:
    ...
    try:
        ...
        # not enough entries to perform smoothing
        return
    ...
    # avoid spiking
    for ii in range(1, len(smoothed)-1):
        if ((((smoothed[ii-1] - smoothed[ii]) > 0.2) or ((smoothed[ii-1] - smoothed[ii]) < -0.2)) and 
            (((smoothed[ii] - smoothed[ii +1]) > 0.2) or ((smoothed[ii] - smoothed[ii +1]) < -0.2)) ):
            smoothed[ii] = 0.5 * (smoothed[ii-1] + smoothed[ii+1])

    # --- Write smoothed NDVI values at daily scale ---
    base_idx_start = (start_date - base_date).days if 'base_date' in globals() else (start_date - zarr_date_to_date(dates_list[0])).days
    base_idx_end = (end_date - base_date).days if 'base_date' in globals() else (end_date - zarr_date_to_date(dates_list[0])).days
...
# =========================
# The MAIN continous_ndvi function (exactly as you provided)
# =========================
def continous_ndvi(day_date, pixel_idx):
    ...
    if true_value:
        ...
        if potential_date != date(1900,1,1):
            ...
            potential_ndvi = ndvi_zarr[pd_idx, pixel_idx] if 0 <= pd_idx < ndvi_zarr.shape[0] else np.int16(0)
...
# =========================
# Worker: process one zarr file
# =========================
def process_file(file_path):
    try:
        file_name = os.path.basename(file_path)
        out_path = os.path.join(OUTPUT_DIR, file_name)
        # copy to output (overwrite if exists)
        if os.path.exists(out_path):
            shutil.rmtree(out_path)
        shutil.copytree(file_path, out_path)
        print(f"[{file_name}] copied to {out_path}")
        ...
        n_days_available = ndvi_zarr.shape[0]
        days_to_run = min(DAYS_TO_PROCESS, n_days_available) if DAYS_TO_PROCESS is not None else n_days_available

        # Main loop: outer loop over days, inner over selected pixels — matches your original
        for di in range(days_to_run):
            day_date = dates_list[di]
            for pixel_idx in selected_pixels:
                continous_ndvi(day_date, pixel_idx)
            if di % 250 == 0:
                print(f"[{file_name}] processed day {di}/{days_to_run}")

        gc.collect()
        print(f"[{file_name}] finished successfully")
        return {"file": file_name, "status": "ok", "processed_pixels": int(len(selected_pixels)), "days": int(days_to_run)}
...
# =========================
# Run parallel over first N_FILES zarrs
# =========================
if __name__ == "__main__":
    ...
    print(f"Starting processing of {len(zarr_files)} files with {N_PIXELS_PER_FILE} pixels each (copying to {OUTPUT_DIR})")

    results = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=N_FILES) as executor:
        futs = {executor.submit(process_file, p): p for p in zarr_files}
