...
OUT_ZARR  = "/data_3/scratch/francesco/new_zarr.zarr"
...
sel_1 = extract_pixel_index(
    center_x - 6500, center_y - 6500, # TODO: if x and y are Swiss coordinates they increas north and eastward.
    center_x + 6500, center_y + 6500) #       Thus the provided coordinates are lower-left (SW) and upper-right (NE).
...
last_dates_da = xr.DataArray(
    da.full((8, ndvi_daily.sizes["pixel"]), fill_value=b"1900-01-01", dtype="S10"),
    dims=("last_date_idx", "pixel"),
    coords={"last_date_idx": np.arange(8)},
)
