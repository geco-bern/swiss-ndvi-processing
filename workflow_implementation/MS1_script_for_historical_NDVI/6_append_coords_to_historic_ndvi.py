...
#### The v3 data structure is the following:
...
HISTORIC_ZARR    = "/data_3/scratch/francesco/ndvi_processed_all_pixels.zarr"
HISTORIC_ZARR_v3 = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3.zarr"
...
ndvi_hist   = xr.open_zarr(HISTORIC_ZARR)
#ndvi_hist3  = xr.open_zarr(HISTORIC_ZARR_v3)
...
trans = from_origin(left, top, px, px)
# xy(trans, [0], [0])    # returns center coordinates of first upper left pixel
# xy(trans, [10], [10])  # returns center coordinates of tenth upper left pixel (i.e. is more east and more south)
# xy(trans, [0, 10], [0, 10]) # this goes from pixel index to coordinate

# rows, cols = np.nonzero([[1, 0, 1],[0, 0, 0],[0, 0, 0]]) # returns [0 0] and [0 2]

rows, cols = np.nonzero(forest_mask)
ids = np.arange(len(rows))
xs, ys = xy(trans, rows, cols)
...
ndvi_hist_v3 = ndvi_hist.assign_coords(
...
for it in range(5,6): # only run case 5 which appears to be fast and yield good compression
    match it:
        case 0:
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_uncompressed.zarr"
            compressor = None
        case 1:
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compressed.zarr"
            compressor = zarr3.Blosc() # see https://github.com/pydata/xarray/issues/9987#issuecomment-2631471771
        case 2:
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compressed_zstd31.zarr"
            compressor = zarr3.Blosc(cname="zstd", clevel=3, shuffle=1)
        case 3:
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compressed_zstd91.zarr"
            compressor = zarr3.Blosc(cname="zstd", clevel=9, shuffle=1)
        case 4:
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compressed_zstd92.zarr"
            compressor = zarr3.Blosc(cname="zstd", clevel=9, shuffle=2)
        case 5:
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compressed_zstd32.zarr"
            HISTORIC_ZARR_v3_compr = "/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compr.zarr"
    ...
    t0=time.perf_counter()
    out_ds.chunk(           # for  testing add before .chunk(): .isel(pixel = range(0,1000000))
        {"pixel": 500000, "date": 30}
        ).to_zarr(
            HISTORIC_ZARR_v3_compr,
            mode="w",
            consolidated=True,
            compute=True,
...
# =====================================================
#xr.open_zarr("/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compressed_zstd32.zarr")
#xr.open_zarr("/data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compr.zarr")

# ndvi_hist3    = xr.open_zarr(HISTORIC_ZARR_v3)
# ndvi_hist3    = xr.open_zarr(HISTORIC_ZARR_v3_compr)
...
# ssh dash
# tmux
# rsync --dry-run -a --human-readable --progress -i -e 'ssh -p 2222' /data_3/scratch/francesco/ndvi_processed_all_pixels_v2.zarr fabian-bernhard@dac3.ddns.net:/mnt/data1/UniBe-swiss-ndvi/data/
# rsync --dry-run -a --human-readable --progress -i -e 'ssh -p 2222' /data_3/scratch/francesco/ndvi_processed_all_pixels_v3.zarr fabian-bernhard@dac3.ddns.net:/mnt/data1/UniBe-swiss-ndvi/data/
# rsync --dry-run -a --human-readable --progress -i -e 'ssh -p 2222' /data_3/scratch/francesco/ndvi_processed_all_pixels_v3_compr.zarr fabian-bernhard@dac3.ddns.net:/mnt/data1/UniBe-swiss-ndvi/data/
